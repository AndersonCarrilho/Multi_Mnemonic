...
def main(stdscr):
    ...
    # Iniciar o processo de monitoramento e limpeza de cache
    processo_monitoramento = mp.Process(target=monitorar_e_limpar_cache)
    processo_monitoramento.start()

    while True:
        stdscr.clear()
        stdscr.addstr(0, 0, "Escolha o tamanho das frases aleatórias que você deseja gerar:")
        stdscr.addstr(1, 0, "1 - Frases de 12 palavras")
        stdscr.addstr(2, 0, "2 - Frases de 18 palavras")
        stdscr.addstr(3, 0, "3 - Frases de 24 palavras")
        stdscr.refresh()
        ...
        if opcao == '1':
            ...
            stdscr.addstr(4, 0, "Opção inválida. Por favor, escolha 1, 2 ou 3.")
            stdscr.refresh()
            time.sleep(2)
            continue

        # Gerar frases aleatórias de forma sequencial
        gerar_frases_aleatorias(stdscr, num_palavras, word_list)

    processo_monitoramento.terminate()  # Finaliza o processo de monitoramento
